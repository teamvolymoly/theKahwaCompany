...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      ...
      setDashboardError("");
      try {
        const [dashboardRes, addressRes, ordersRes, reviewsRes] =
          await Promise.all([
        ...
        setStats([
          {
            label: "Total orders",
            value: String(nextStats.total_orders ?? orderItems.length ?? 0),
          },
          {
        ...
        setRecentOrders(
          orderItems
            .slice(0, 5)
            .map((order) => normalizeOrder(order, currency)),
        );
        const eligibleItems = extractEligibleReviewItems(reviewsRes);
        setReviewItems(
          eligibleItems.filter((item) => item.can_review).slice(0, 4),
        );
        setAddresses(
          Array.isArray(addressRes) ? addressRes : addressRes?.data || [],
        );
      } catch (err) {
        if (active) {
  ...
  return (
    <main className="min-h-screen bg-white text-black mt-14">
      <section className="site-container py-14">
        ...
        {dashboardLoading && (
          <p className="mt-6 text-md text-black/60">Loading dashboard...</p>
        )}
        ...
        <div className="mt-10 rounded-sm border border-black/10 bg-white p-6 shadow-sm">
          ...
          {reviewItems.length > 0 ? (
                ...
                >
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-md text-black/45">
            ...
            <p className="mt-4 text-md text-black/60">
              Delivered products that can be reviewed will appear here. You can
              also open any delivered order to review its items.
            </p>
          )}
        </div>
              ...
              {recentOrders.map((order) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-md text-black/50">{order.date}</p>
                  ...
                  <div className="text-md text-black/60 md:text-center">
                    ...
                    {isPaidDeliveredOrder(order) && (
                      <p className="mt-2 text-md font-semibold uppercase tracking-[0.08em] text-black">
                        Give review
                      </p>
                    )}
                  </div>
                  <div className="text-md font-semibold">{order.total}</div>
                </Link>
              ))}
              {!dashboardLoading && recentOrders.length === 0 && (
                <p className="text-md text-black/60">No recent orders yet.</p>
              )}
            </div>
          </div>
          ...
          <div className="py-6">
            ...
            {defaultAddress ? (
              <div className="mt-4 rounded-sm shadow-sm bg-gray-50 p-4">
                ...
                <p className="mt-2 text-md">{defaultAddress.address_line1}</p>
                <p className="text-md">{defaultAddress.address_line2}</p>
                <p className="text-md">
                  {defaultAddress.city}, {defaultAddress.state}{" "}
                  {defaultAddress.pincode}
                </p>
                <p className="text-md">{defaultAddress.country}</p>
              </div>
            ) : (
              <p className="mt-4 text-md text-black/60">
